...
class PointTracker(object):
    ...
    def get_tracklets_with_movement(self, tracks, min_distance=4):
        ...
        for track in tracks:
            ...
            for i in range(N - 1):
                ...
                track_data.append((p1, p2))

            if len(track_data) >= 2:
                start, end = track_data[0][0], track_data[-1][1]
                if np.sum((np.array(end) - np.array(start)) ** 2) > min_distance ** 2:
                    moving_tracks.append({"id": track[0], "score": track[1], "start": start, "end": end})

        return moving_tracks


    def draw_tracklets(self, out, tracks, show_points = True, show_tracks = True):
        stroke = 2

        if show_tracks:
            for track in tracks:
                clr = myjet[int(np.clip(np.floor(track["score"] * 10), 0, 9)), :] * 255
                cv2.line(out, track["start"], track["end"], clr, thickness=stroke, lineType=16)
        ...
        if show_points:
            for track in tracks:
                ...
                cv2.circle(out, track["end"], stroke, clr2, -1, lineType=16)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hide_tracks', action='store_true',
                        help='Hide tracks (default: False)')
    parser.add_argument('--hide_points', action='store_true',
                        help='Show end points of tracks (default: False)')
    ...
    while True:
        ...
        tracker.draw_tracklets(out1, points, show_tracks=not opt.hide_tracks, show_points= not opt.hide_points)
        if opt.show_extra:
